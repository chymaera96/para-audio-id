...
from para_audio_id.audio_lm.training import (
    validate_tc14_batch_configuration,
    validate_tc14_query_configuration,
)
...
def test_primary_config_is_audio_lm_and_matches_logical_batch():
    ...
    assert cfg["tokenizer"]["selected_codebooks"] == 3
    ...
    assert cfg["train"]["id_digit_weight"] == 30.0
...
def test_tc14_startup_query_invariants_are_enforced():
    cfg = resolve_training_config(
        yaml.safe_load(
            (Path(__file__).parents[1] / "configs" / "fma_large.yaml").read_text()
        )
    ...
    query_spec = validate_tc14_query_configuration(
        cfg, tokenizer_spec, AudioLMVocabulary(num_codebooks=3)
    )
    ...
    assert query_spec["audio_targets"] == 375
    ...
    assert batch_spec["audio_targets_per_microbatch"] == 30_000
    assert batch_spec["causal_tokens_per_microbatch"] == 30_640
    ...
    with pytest.raises(ValueError, match="segment_duration=5.0"):
        validate_tc14_query_configuration(
            wrong_duration, tokenizer_spec, AudioLMVocabulary(num_codebooks=3)
        )

    wrong_weight = deepcopy(cfg)
    wrong_weight["train"]["id_digit_weight"] = 20.0
    with pytest.raises(ValueError, match="id_digit_weight=30"):
        validate_tc14_query_configuration(
            wrong_weight, tokenizer_spec, AudioLMVocabulary(num_codebooks=3)
        )


def test_tc14_rejects_two_codebook_query_profile():
    with pytest.raises(ValueError, match="three MuQ codebooks"):
