...
from para_audio_id.audio_lm.training import (
    validate_tc15_batch_configuration,
    validate_tc15_query_configuration,
)
...
def test_primary_config_is_audio_lm_and_matches_logical_batch():
    ...
    assert cfg["train"]["id_digit_weight"] == 40.0
    ...
    assert (
        ...
        == "tc15-four-codebook-logit-distillation"
    )
    assert (
        cfg["train"]["distillation"]["protocol"]
        == "tc15_four_codebook_logit_distillation_v1"
    )
...
def test_tc15_startup_query_invariants_are_enforced():
    cfg = resolve_training_config(
        yaml.safe_load(
            (Path(__file__).parents[1] / "configs" / "fma_large.yaml").read_text()
        )
    ...
    assert query_spec["audio_targets"] == 500
    assert query_spec["digit_targets"] == 5
    assert query_spec["boundary_targets"] == 2
    assert query_spec["document_tokens"] == 508
    prompt = prompt_from_audio_tokens(
        torch.zeros(500, dtype=torch.long), AudioLMVocabulary(num_codebooks=4)
    )
    assert len(prompt) == 502
    assert len(prompt) + 6 == 508
    assert len(prompt) + 6 <= cfg["model"]["max_position_embeddings"]
    assert batch_spec["documents_per_microbatch"] == 80
    assert batch_spec["audio_targets_per_microbatch"] == 40_000
    assert batch_spec["causal_tokens_per_microbatch"] == 40_640
    assert batch_spec["waveform_seconds_per_microbatch"] == 400.0
    assert batch_spec["tracks_per_optimizer_step"] == 80
    assert batch_spec["documents_per_optimizer_step"] == 160
    assert batch_spec["audio_targets_per_optimizer_step"] == 80_000
    ...
    with pytest.raises(ValueError, match="40 tracks per microbatch"):
        validate_tc15_batch_configuration(wrong_batch, query_spec)
    ...
    with pytest.raises(ValueError, match="id_digit_weight=40"):
        validate_tc15_query_configuration(
            wrong_weight, tokenizer_spec, AudioLMVocabulary(num_codebooks=4)
        )


def test_tc15_rejects_three_codebook_query_profile():
