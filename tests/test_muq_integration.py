...
def test_real_muq_tc16_eighty_document_probe():
    ...
    assert report["serialized_tokens_per_example"] == 200
    ...
    assert lightweight_tokens.shape == (80, 200)
    cfg = {
        "model": {
            "architecture": "gpt2",
            "num_layers": 1,
    ...
    assert batch["input_ids"].shape == (80, 208)
    assert int(batch["audio_target_mask"].sum()) == 16_000
    logits = model(
